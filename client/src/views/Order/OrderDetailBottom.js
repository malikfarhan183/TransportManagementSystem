...
let bRow=""
let cRow=""
class OrderDetailBottom extends React.Component {
  handleNextBtn=e=>{
...
EventBus.on("nextBtn", this.handleNextBtn)
  }
  toggleModal=(e)=> {
...
this.setState(prevState => ({
  ...
  render() {
    return (
             ...
             <div>
                <Nav style={{height: '44px',  display: 'flex',alignItems: 'center',justifyContent: 'center'}}tabs>
                  <Row style = {{width:'100%',height: '25px'}}>
                  <NavItem className="col-md-3" style={{ paddingRight: '23px',paddingLeft: '0px'}}>
                    <NavLink
                      style= {{cursor:'pointer', height:'70px',background: 'white',textAlign:'center'}}
                        className={classnames({ tabActive: this.state.activeTab === '1' }, "tabNotActive") }
                      onClick={() => { this.toggle('1'); }}
                    >
                      <p style={tabStyle1}>Verification</p>
                    </NavLink>
                  </NavItem>

              {/*    <NavItem className="col-md-3"  style={{paddingRight: '16px',paddingLeft: '10px'}}>
                    <NavLink
                      style= {{cursor:'pointer',height:'70px',textAlign:'right' ,background: 'white',textAlign:'center'}}
                        className={classnames({ tabActive: this.state.activeTab === '2' }, "tabNotActive") }
                      onClick={() => {

                        this.toggle('2'); }}
                    >
                  <p style={tabStyle2}>Distribution</p>
                  </NavLink>
                  </NavItem>

                  <NavItem className="col-md-3"  style={{paddingLeft:'18px',paddingRight: '10px'}}>
                    <NavLink
                      style= {{cursor:'pointer', height:'70px',background: 'white',textAlign:'center'}}
                      className={classnames({tabActive: this.state.activeTab === '3' },"tabNotActive") }
                      onClick={() => { this.toggle('3'); }}
                    >
                        <p style={tabStyle3}>Collection</p>
                    </NavLink>
                  </NavItem>

                  <NavItem  className="col-md-3"  style={{paddingRight:'1px',paddingLeft: '24px'}}>
                    <NavLink
                      style= {{cursor:'pointer', height:'70px',background: 'white',textAlign:'center'}}
                      className={classnames({tabActive: this.state.activeTab === '4' },"tabNotActive") }
                      onClick={() => { this.toggle('4'); }}
                    >
                      <p style={tabStyle4}>Deposite</p>
                    </NavLink>
                  </NavItem>*/}
